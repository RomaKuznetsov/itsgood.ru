...
public class BucketDataController {
    ...
    public ResponseEntity<BucketDTO> updateBucket (@Validated @RequestBody BucketRequestUpdate request, BindingResult result) {
        if (result.hasErrors()) {
            throw new IllegalRequestException(result);
        }
    ...
    @DeleteMapping(value = "/deleteBucket", consumes = {"application/xml", "application/json"})
    public ResponseEntity<Object> deleteBucket(@Validated @RequestBody BucketRequestUpdate request, BindingResult result) {
        if (result.hasErrors()) {
            throw new IllegalRequestException(result);
        }
        bucketDataService.deleteBucket(request);
        return new ResponseEntity<>(HttpStatus.OK);
    }
}
